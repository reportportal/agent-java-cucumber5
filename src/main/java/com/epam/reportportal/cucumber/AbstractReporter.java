...
public abstract class AbstractReporter implements ConcurrentEventListener {
	...
	protected void afterScenario(TestCaseFinished event) {
		...
		List<Pair<Pair<String, URI>, RunningContext.ScenarioContext>> mapEntries = currentScenarioContextMap.entrySet()
				.stream()
				.map(e -> Pair.of(e.getKey(), e.getValue()))
				.collect(Collectors.toList());
		for (Pair<Pair<String, URI>, RunningContext.ScenarioContext> scenarioContext : mapEntries) {
	...
	protected void handleStartOfTestCase(TestCaseStarted event) {
		...
		RunningContext.FeatureContext newFeatureContext = new RunningContext.FeatureContext(event.getTestCase());
