...
package com.epam.reportportal.cucumber.integration.embed.text;
...
public class EmbeddingStepdefs {
	...
	@Given("I have a dummy step to attach a text with incorrect mime type")
	public void i_have_a_dummy_step_to_make_a_screenshot_incorrect_type() throws InterruptedException {
		type = "image/png";
		Thread.sleep(CommonUtils.MINIMAL_TEST_PAUSE);
	}

	@Given("I have a dummy step to attach a text with partially correct mime type")
	...
	public void embedAnImage(Scenario scenario) throws IOException {
		scenario.embed(
				IOUtils.toByteArray(Objects.requireNonNull(getClass().getClassLoader().getResourceAsStream("files/plain.txt"))),
				type,
				"ignored"
		);
	}
